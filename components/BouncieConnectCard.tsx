...
export default function BouncieConnectCard({ token }: { token: string }) {
  ...
  // Stale = token in DB but expired and the auto-refresh isn't fixing it.
  // Show a clear "Reconnect" CTA instead of the silent green check that
  // hides the problem.
  if (status.stale) {
    return (
      <div className="flex items-center justify-between gap-2 rounded-xl border border-amber-900/60 bg-amber-950/30 px-3 py-2 text-xs text-amber-200">
        <div className="flex items-center gap-2">
          <WifiOff size={14} /> Bouncie token expired — reconnect to resume live data
        </div>
        <a
